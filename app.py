...
def video_feed():
    return Response(detect('rtsp://admin:admin@10.202.21.124:8554/live'),
...
def processV():
    print("处理视频")
    a = processVideo('E:\\programing\\backend\\sysbackend\\uploads\\testrim.mp4')
    a.detect()
    # return send_file(r'E:\programing\backend\sysbackend\uploads\ScreenLock.jpg', mimetype='image/gif')
    jsonData = {
        'isProcess': True,
        'videourl': 'http://127.0.0.1:5000/static/testrim.mp4'
    }
    return jsonify(jsonData)

@app.route('/checkPV')
def checkPV():
    exist = os.path.exists(r'E:\programing\backend\sysbackend\inference\output2\testrim.mp4')
    print("文件" + str(exist))
    return 'ok'
...
@app.route('/download')
def downloadFile():
    # res = make_response(send_file(r'static/results.txt',  '行人轨迹信息.txt', as_attachment=True))
    # res.headers.add('Access-Control-Allow-Origin', '*')
    # return res
    return send_file(r'static/results.txt', 'trackInfo.txt', as_attachment=True)


@app.route('/setRtspUrl')
def setRstpUrl():
    Rurl = request.args.get('rtspUrl')
    return Response(detect(Rurl), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/getPeopleNum')
def getPeopleNum():
    global peopleNum
    jsonData = {
        'peopleNum': peopleNum
...
@app.route('/assets')
def getErrPhoto():
    return send_file('./assets/error.png', mimetype='image/gif')
...
def detect(sourceUrl, save_img=False):
